...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    private float semiWeaponCooldownTime = 3f;
    ...
    private void HandleDash() 
    {
        if (Input.GetKey(KeyCode.Q))
        {
            ...
            holdTime = Mathf.Clamp(holdTime, 0, requiredHoldTime);
            Debug.Log(holdTime);
        }

        // When releasing Ctrl, apply dash in the player's forward direction
        if (Input.GetKeyUp(KeyCode.Q))
        {
            Vector3 dashDirection = transform.forward * holdTime * 500f; // Move forward
            Debug.Log("Dash!");
